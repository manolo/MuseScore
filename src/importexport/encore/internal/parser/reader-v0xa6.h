...
struct EncFormatReader_V0xA6 final : EncFormatReader
{
    ...
    bool postProcessElement(EncMeasureElem* elem,
                            QDataStream& ds,
                            qint64 rawElemStart) const override;

    bool deduplicateRest(std::vector<std::unique_ptr<EncMeasureElem> >& elements,
                         EncMeasureElem* candidate) const override;
    ...
    void readKeyFromTKBlock(EncInstrument& instr,
                            QDataStream& ds,
                            qint64 contentStart) const override;
};
