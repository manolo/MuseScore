...
void resolveSlurs(BuildCtx& ctx)
{
    ...
    // The v0xC2 slur measure-count (element +16) is trustworthy now that the reader selects its byte
    // by format version. One tell remains: a count pointing past the last measure is impossible, so it
    // marks the whole file's field as junk and every slur then resolves by the xoffset heuristic.
