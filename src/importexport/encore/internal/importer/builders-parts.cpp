...
{
    // Part::partName() is a stored value written only by initFromInstrTemplate, which this importer
    // never calls, so naming the instrument alone leaves it empty and the Timeline panel and any
    // generated excerpt name come out blank. Set it from the Encore name for every path below.
    if (!instr.name.isEmpty()) {
        part->setPartName(String(instr.name));
    }
    ...
    if (tmpl) {
        // A bare MIDI-program match lands on the generic family template ("recorder"); prefer its
        // listed named sibling ("Soprano Recorder") so the instrument identity is not left generic.
