                                                       ...
                                                       bool* outUniqueName)
{
    ...
    for (const InstrumentGroup* g : instrumentGroups) {
        for (const InstrumentTemplate* it : g->instrumentTemplates) {
            ...
            const QString nl = normalizeForCompare((it->longNames.empty() ? QString() : it->longNames.front().name().toQString()));
            const QString ns = normalizeForCompare((it->shortNames.empty() ? QString() : it->shortNames.front().name().toQString()));
            int nameStrength = 0;
            bool exactHit = false;
    ...
    for (const InstrumentGroup* g : instrumentGroups) {
        for (const InstrumentTemplate* it : g->instrumentTemplates) {
            ...
            const QString nl = normalizeForCompare(
                (it->longNames.empty() ? QString() : it->longNames.front().name().toQString()));
            const QString ns = normalizeForCompare(
                (it->shortNames.empty() ? QString() : it->shortNames.front().name().toQString()));
            int score = 0;
            for (const QString& needle : needles) {
                if (nt == needle) {
                    score += 4;
...
{
    return !it->trackName.isEmpty() && !it->longNames.empty();
}
