...
| 0xBE    | ACCENT        | standalone accent above (>) in v0xC4 as size-16 ORN; maps to articAccentAbove. In v0xC2, accent is instead ORN tipo=0xC4. **Two parsing rules:** (1) **Voice scan** — the ORN `voice` byte is always 0 regardless of which voice the annotated note is in; scan all four voices of the ORN's staff before falling back to the sibling staff, otherwise accents on staves where notes are all in voice 1+ are missed. (2) **Tick derivation** — the target tick is `measStartTick + orn.tick` (raw Encore tick), not the cumulative placement tick. When voice 0 has no notes on the staff, the cumulative tick stays at 0 throughout the measure, making the cumulative tick useless for positioning; the raw ORN tick always gives the correct beat. (3) **Xoffset pre-check**: before applying any tick correction, the importer checks whether a note at the ORN's raw Encore tick has an xoffset within `BOW_XOFF_CLUSTER` (6 units) of the ORN's own xoffset; if so, the ORN is already at its correct chord and no correction is applied. This guards against the case where two marks in the same measure share the same relative xoffset value (common because xoffset is relative to the note head, not an absolute page position). Same rules apply to DOWNBOW and UPBOW. |
