...
Not every v0xC2 file uses +16 as a count, though: some store noise there, mixing plausible small values with out-of-range sentinels (0xFE/0xFF) even though every slur is a within-bar arc. A count that points past the last measure cannot be a real span, so when any slur in the file carries one, the +16 field is unreliable for that whole file and every slur should be resolved by the xoffset heuristic instead. A second unreliable pattern is a per-staff **constant**: some files store the same value at +16 for every slur on a staff (for example 13 on the top staff and 11 on the bottom) regardless of where the slur starts. These values stay in range, so the past-the-end test misses them, but a genuine forward count varies from slur to slur; the same multi-measure value (3 or more) repeated across slurs that start in different measures marks +16 unreliable for the file too.
